...
set_options <- function(csv_filename, daymet_variables = "tmax,tmin,srad,vp,swe,prcp,dayl", min_lon = 0, max_lon = 0, min_lat = 0, max_lat = 0, region = "na") {
  # csv_filename: Input CSV file, containing (at minimum) columns of id, lat, lon, start_date, and end_date. Additional columns may be included as well.
  # daymet_variables: [Optional] Comma-separated string of Daymet variables: "tmax,tmin,srad,vp,swe,prcp,dayl". Default is to download and link all Daymet variables.
  ...
  out <- list("csv_filename" = csv_filename, "daymet_variables" = daymet_variables, "min_lon" = min_lon, "max_lon" = max_lon, "min_lat" = min_lat, "max_lat" = max_lat, "region" = region)
  return(out)  
}

# Specifying user-customized options
#### DO NOT CHANGE ANYTHING BEFORE THIS ####
set_options_out <- set_options(csv_filename = "loyalty_degauss.csv",
...
import_data <- function(.csv_filename = csv_filename, .min_lon = min_lon, .max_lon = max_lon, .min_lat = min_lat, .max_lat = max_lat, .region = region) {
